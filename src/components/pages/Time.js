...
function Time() {
  ...
  const dispararSirene = async () => {
    try {
      ...
      const response = await fetch(`http://${esp8266IP}/tocar`, { method: 'GET' });
      if (response.ok) {
  ...
  return (
    <div className={styles.project_container}>
      ...
      <Container customClass="column">
        {times.length > 0 && times.map((time) => <TimeCard key={time.id} id={time.id} time={time.time} handleRemove={removeTime} />)}
